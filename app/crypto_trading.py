...
class CryptoState:
    ...
    def __init__(self):
        ...
        if True:
            ### Restore from file if it does
            return
        else:
            ### Prep table data if it does not
            return

    def __setattr__(self, name, value):
        ### Make sure that backup is written every time the attr is updated
        return
...
def buy_alt(client, alt_symbol, crypto_symbol):
    ...
    order_quantity = math.floor(
        get_currency_balance(client, crypto_symbol)
        * 10 ** ticks[alt_symbol]
...
def update_trade_threshold(client):
    ...
    current_coin_price = get_market_ticker_price_from_list(
        all_tickers, g_state.current_coin + BRIDGE
    )

    if current_coin_price is None:
        logger.info(
            "Skipping update... current coin {0} not found".format(
                g_state.current_coin + BRIDGE
            )
        )
        return

    for coin_dict in g_state.coin_table.copy():
        coin_dict_price = get_market_ticker_price_from_list(
            all_tickers, coin_dict + BRIDGE
        )

        if coin_dict_price is None:
            logger.info(
                "Skipping update for coin {0} not found".format(coin_dict + BRIDGE)
            )
            continue

        g_state.coin_table[coin_dict][g_state.current_coin] = (
            coin_dict_price / current_coin_price
        )
...
def initialize_trade_thresholds(client):
    ...
    for coin_dict in g_state.coin_table.copy():
        ...
        if coin_dict_price is None:
            logger.warning(
                "Skipping initializing {0}, symbol not found".format(coin_dict + BRIDGE)
            )
            continue
        ...
        for coin in supported_coin_list:
            ...
            if coin != coin_dict:
                coin_price = get_market_ticker_price_from_list(
                    all_tickers, coin + BRIDGE
                )
...
def scout(client, transaction_fee=0.001, multiplier=5):
    ...
    for optional_coin in [
        coin
        for coin in g_state.coin_table[g_state.current_coin].copy()
        if coin != g_state.current_coin
    ]:
        optional_coin_price = get_market_ticker_price_from_list(
            all_tickers, optional_coin + BRIDGE
        )

        if optional_coin_price is None:
            logger.info(
                "Skipping scouting... optional coin {0} not found".format(
                    optional_coin + BRIDGE
                )
            )
            continue
        ...
        if (
            ...
            logger.info(
                "Will be jumping from {0} to {1}".format(
